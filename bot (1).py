...
#loads everything needed from the .env
load_dotenv()
DISCORD_TOKEN = os.getenv("BOT_TOKEN")
urls = [os.getenv("SITE_ONE"), os.getenv("SITE_TWO")]
divs = [os.getenv("DIV_ONE"), os.getenv("DIV_TWO")]
div_types = [os.getenv("DIV_TYPE_ONE"), os.getenv("DIV_TYPE_TWO")]
titles = [os.getenv("TITLE_ONE"), os.getenv("TITLE_TWO")]
title_types = [os.getenv("TITLE_TYPE_ONE"), os.getenv("TITLE_TYPE_TWO")]
links = [os.getenv("LINK_ONE"), os.getenv("LINK_TWO")]
files = [os.getenv("STATE_FILE_ONE"), os.getenv("STATE_FILE_TWO")]
prefixes = [os.getenv("PREFIX_ONE"), os.getenv("PREFIX_TWO")]

intents = discord.Intents.all()
intents.message_content = True
bot = commands.Bot(command_prefix='!', intents=intents)

#loads the latest title from a file
def load_state(i):
...
#saves the latest title to a file
def save_state(title,i):
    state = {"latest_title": title}
    with open(files[i], 'w') as f:
        json.dump(state, f)

#creates an embed to report a new article
...
#checks for new articles every ten minutes
